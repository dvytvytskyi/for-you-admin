#!/bin/bash
# Виконайте ці команди на сервері вручну

cd /root/admin-panel

echo "🗄️  Створення таблиці токенів..."

# Використовуємо правильну назву контейнера з ID
docker exec -i 2f11b1ef7b08_for-you-admin-panel-postgres-prod psql -U admin -d admin_panel << 'SQL'
...
echo ""
echo "🔄 Перезапуск backend (якщо є контейнер)..."

# Перевіряємо чи є backend контейнер
BACKEND_CONTAINER=$(docker ps --format "{{.Names}}" | grep -i backend | head -1)
if [ -n "$BACKEND_CONTAINER" ]; then
  echo "   Перезапускаю: $BACKEND_CONTAINER"
  docker restart "$BACKEND_CONTAINER"
  sleep 10
  echo "✅ Backend перезапущено"
else
  echo "   ⚠️  Backend контейнер не знайдено (можливо запущений окремо)"
  echo "   Перезапустіть backend вручну якщо потрібно"
fi

echo ""
echo "🔑 Встановлення токенів..."
...
if [ -z "$TOKEN" ]; then
  echo "❌ Не вдалося отримати admin token"
  echo "Перевірте чи працює backend"
else
  echo "   Token отримано, встановлюю токени..."

...
const UNITS_CACHE_FILE = '/root/reelly_units_parsed_MEGA.json';
...
async function run() {
  ...
  for (const rp of projects) {
    ...
    for (const dp of dbProps) {
      const dNameFull = normalizeFull(dp.name);
      const dNameSimple = normalizeSimple(dp.name);
      
      if (dNameFull.includes(rNameSimple) || rNameSimple.includes(dNameSimple) || dNameSimple.includes(rNameSimple)) {
        list.push(dp.id);
      }
    }
    if (list.length > 0) reellyToDb.set(rp.id, list);
  }
  console.log(`🧩 Aggressive matches: ${reellyToDb.size} / ${projects.length} Reelly projects.`);

  // 2. Unit Parsing with local Cache
  let cache = [];
  try {
     if (fs.existsSync(UNITS_CACHE_FILE)) { 
       cache = JSON.parse(fs.readFileSync(UNITS_CACHE_FILE, 'utf8')); 
     }
  } catch (e) {}
  
  const cacheNames = new Set(cache.map(c => c.propertyName.toLowerCase()));
  console.log(`📦 Cache: ${cache.length} projects already fetched.`);

  for (let i = 0; i < projects.length; i++) {
    const rp = projects[i];
    if (cacheNames.has(rp.name.toLowerCase())) continue; 

    // ONLY fetch projects that ARE MATCHED in DB or are ON SALE
    if (!reellyToDb.has(rp.id)) continue; 

    process.stdout.write(`[${i+1}/${projects.length}] Fetching ${rp.name} (#${rp.id})... `);
    try {
      const units = [];
      ...
      cache.push({ propertyName: rp.name, reellyId: rp.id, units: units });
      
      if (i % 10 === 0) fs.writeFileSync(UNITS_CACHE_FILE, JSON.stringify(cache, null, 2));
    } catch (e) {
      console.log(`ERROR: ${e.message}`);
      if (e.message.includes('401')) {
         console.log('Token expired. Stopping fetch loop.');
         break;
      }
    }
    await sleep(250);
  }
  fs.writeFileSync(UNITS_CACHE_FILE, JSON.stringify(cache, null, 2));

  // 3. Final Push to DB
  console.log('\n🚀 Starting Heavy DB Push...');
  const dbUnitsBundle = new Map();
  for (const c of cache) {
    const dbIds = reellyToDb.get(c.reellyId);
    if (!dbIds) continue;
    for (const dbId of dbIds) {
      if (!dbUnitsBundle.has(dbId)) dbUnitsBundle.set(dbId, []);
      dbUnitsBundle.get(dbId).push(...c.units);
    }
  }
  ...
  for (let j = 0; j < bundledIds.length; j++) {
    ...
    process.stdout.write(`[${j+1}/${bundledIds.length}] Updating ${dbId}: ${uniqueUnits.length} units... `);
